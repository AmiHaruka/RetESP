...
LastEditors:  Kanbe 
LastEditTime: 2025-04-19 23:03:53
FilePath: /RetESP/MolFormer+RetNet/train.py
Description: 
'''
...
mp.set_forkserver_preload([])  # 避免资源泄漏
...
class LazyDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 实时生成分子特征
        #smi_tensor = torch.zeros(self.max_smiles_len, dtype=torch.long)
        smi = item['smiles']
        #smi_tensor[:len(smi)] = torch.tensor(smi)
...
def collate_fn(batch, device):
    labels, seqs, smiles = zip(*batch)
    labels = torch.stack(labels).to(device)  
    seqs   = torch.stack(seqs).to(device)    
    return labels, seqs, list(smiles)
...
def load_datasets(args, phase):
    ...
    def create_loader(data_path, shuffle=False):
        ...
        return DataLoader(
            ...
            collate_fn=lambda b: collate_fn(b, device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
            )
        )
...
def train_epoch(model, optimizer, train_loader, device, loss_fn):
    ...
    for labels, seq, smiles in train_loader:
...
def run_validation(model, val_loader, loss_fn, device):
    ...
    with torch.no_grad():
        for labels, seq, smiles in val_loader:
            seq = seq.to(device)
            smiles = smiles.to(device)
            labels = labels.to(device).float()
...
def main():
    ...
    parser.add_argument('--ffn_size', type=int, default=512, help='FFN层维度')
    ...
    if args.mode in ['train', 'both']:
        ...
        if args.resume and (Path(args.model_dir)/'best_model.pth').exists() :
            ckpt = torch.load(Path(args.model_dir)/'best_model.pth', map_location=device,weights_only=False)
